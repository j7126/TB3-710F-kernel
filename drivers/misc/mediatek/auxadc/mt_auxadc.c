...
//lidianlong add start 20150515
int IMM_GetOneChannelValueEX(int dwChannel, int deCount)
{
   int i = 0,res =0;
   int data[4] = {0,0,0,0};
   int rawdata=0,rawdata1=0;
   int data0 = 0,data1 = 0;
   long tmo1;
   unsigned int adc_esult=0;
   unsigned int u4Sample_times = 0;

   printk("[adc_driver]: dwChannel=%d\n",dwChannel);

   do
   {
		res = IMM_GetOneChannelValue(dwChannel,data,&rawdata);
		if(res < 0)
		{
   ...
   rawdata1 = rawdata1/deCount;
   data0 = data0/deCount;
   data1 = data1/deCount;

   printk("[adc_driver]:data0 is %d,data1 is %d\n",data0,data1);
   adc_esult = data0*1000+data1*10;
   printk("[adc_driver]: channel[%d]raw =%d\n",dwChannel,rawdata1);
   printk("[adc_driver]: channel[%d]=%d\n",dwChannel,adc_esult);      
   return adc_esult;

}
//lidianlong add  end 20150515
