...
//////end///////////////
	...
	if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_SP2508_MIPI_RAW,currSensorName)))
...
#if 0
	        if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_D,&regVCAMD))
	        {
	            PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
	            //return -EIO;
	            goto _kdCISModulePowerOn_exit_;
		...
		if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
			if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[+++haley+++] sp0a20 set gpio mode failed!! \n");}
			if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMPDN],GPIO_DIR_OUT)){PK_DBG("[+++haley+++] sp0a20 set gpio dir failed!! \n");}
		       if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_OFF])){PK_DBG("[+++haley+++] sp0a20 set gpio failed!! \n");}
			if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_ON])){PK_DBG("[+++haley+++] sp0a20 set gpio failed!! \n");}
		}
