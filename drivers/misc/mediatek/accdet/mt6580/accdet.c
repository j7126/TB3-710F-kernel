...
#define GPIO_HEADSET_SWITCH		GPIO10
...
struct timer_list kkk_first_pult_in;
static int kkk_protect;
...
static void disable_micbias(unsigned long a)
{
	...
	//kkk 
static void kkk_disable_pulg_in(unsigned long a)
{
		//kkk_protect=0;
  	    printk("@@@@the pulg in be protected!!\n");
		mdelay(2);
    ...
    if (cur_eint_state == EINT_PIN_PLUG_IN) {
		...
		//enable ACCDET unit
			...
			{	
				...
				msleep(10);
        	    mt_set_gpio_out(GPIO_HEADSET_SWITCH,GPIO_OUT_ONE);
				msleep(20);
#endif
			}
			else
			{
				msleep(10);
        	    mt_set_gpio_out(GPIO_HEADSET_SWITCH,GPIO_OUT_ZERO);
				msleep(20);
			}
	}else {
//EINT_PIN_PLUG_OUT
...
{
	int ret;
	printk("@@ accdet accdet_workqueue_func\n");
	mdelay(2);
	ret = queue_work(accdet_workqueue, &accdet_work);	
    if(!ret)
    {
...
int mt_accdet_probe(void)	
{
	...
	//*INI kkk for first pulg in
	init_timer(&kkk_first_pult_in);
	kkk_first_pult_in.expires = jiffies + (1*HZ);
	kkk_first_pult_in.function = &kkk_disable_pulg_in;
	kkk_first_pult_in.data = ((unsigned long) 0 );
...
#if defined(HQ_35MM_TI_ENABLE)
    mt_set_gpio_mode(GPIO_HEADSET_SWITCH,GPIO_MODE_00);  // gpio mode
	mt_set_gpio_pull_enable(GPIO_HEADSET_SWITCH,GPIO_PULL_ENABLE);
	mt_set_gpio_dir(GPIO_HEADSET_SWITCH,GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO_HEADSET_SWITCH,GPIO_OUT_ZERO);
#endif
