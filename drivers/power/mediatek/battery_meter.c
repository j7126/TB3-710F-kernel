...
///////////////
extern g_runin_battery_test_flag;
extern g_platform_boot_mode;
///////////
...
kal_int32 gFG_BATT_CAPACITY_aging_High_current = 1200;
...
kal_int32 g_Q_MAX_POS_50[] = {3517, 3559};
kal_int32 g_Q_MAX_POS_25[] = {3528, 3584};
kal_int32 g_Q_MAX_POS_0[]  = {3537, 3484};
kal_int32 g_Q_MAX_NEG_10[] = {3469, 3441};

kal_int32 g_Q_MAX_POS_50_H_CURRENT[] = {3484, 3520};
kal_int32 g_Q_MAX_POS_25_H_CURRENT[] = {3480, 3560};
kal_int32 g_Q_MAX_POS_0_H_CURRENT[]  = {3194, 3301};
kal_int32 g_Q_MAX_NEG_10_H_CURRENT[] = {1974, 2736};

void fgauge_get_profile_id_multi(void)
{
       int g_bat_id =0;
	   int ret;
	   int i;
	   int sum_VVV=0;
       //g_bat_id = IMM_GetOneChannelValueEX(0,5);  
       for(i=0;i<5;i++)
	   	{
	   ret = IMM_GetOneChannelValue_Cali(1, &g_bat_id);
	   //printk("i=%d\n",i);
	   mdelay(3);
	   sum_VVV=g_bat_id+sum_VVV;
		}
	   g_bat_id=sum_VVV/5;
       bm_print(BM_LOG_CRTI, "~~~~~~~~battery id = %d jiale~~~~~~~~~\n",g_bat_id);
	   //mdelay(3);
	   printk("@@@g_bat_id=%d mv\n",g_bat_id);
	   mdelay(3);
       if(g_bat_id < 400000)
       {
                  bm_print(BM_LOG_CRTI, "the battery is atl, id = %d\n",g_bat_id);             
                  g_fg_battery_id = 1;// 1 gong
       }
       else
       {
                  bm_print(BM_LOG_CRTI, "the battery is cos, id = %d\n",g_bat_id);
                  g_fg_battery_id = 0;// 2 gong battery
       }       
}
...
int __batt_meter_init_cust_data_from_dt(void)
{
	...
	while (!of_property_read_u32_index(np, "battery_profile_t3", idx, &addr))
	{
        ...
        battery_profile_t3[g_fg_battery_id][idx/2].voltage=val;
#else
		battery_profile_t3[idx/2].percentage=addr;
		battery_profile_t3[idx/2].voltage=val;
#endif
		if(idx>=saddles*2)
			break;
	}
...
int BattThermistorConverTemp(int Res)
{
	...
	//printk("@@g_fg_battery_id=%d",g_fg_battery_id);
	//mdelay(2);
	if (Res >= batt_temperature_table[0].TemperatureR) {
		//printk("@@i am here11\n");
		//mdelay(3);
		//printk("@@TemperatureR=%d\n",batt_temperature_table[0].TemperatureR);
		TBatt_Value = -20;
		...
		//printk("@@Res=%d,RES2=%d,TMP1=%d,RES1=%d,TMP2=%d\n",Res,RES2,TMP1,RES1,TMP2);
		//mdelay(3);
...
//	mdelay(3);
	printk("low_Q_max=%d,high_Q_max=%d,high_temperature=%d,temperature=%d,low_temperature=%d\n",
		low_Q_max,high_Q_max, high_temperature, temperature, low_temperature);
//	mdelay(5);
...
int BattVoltToTemp(int dwVolt)
{
	...
	if(temp_control_flag == 1){
		sBaTTMP = change_temperature;
		//printk("@@sBaTTMP=%d\n",sBaTTMP);
		//mdelay(2);
		}
	return sBaTTMP;
}
EXPORT_SYMBOL_GPL(BattVoltToTemp);
...
void sw_oam_init_v2(void)
{
	...
	mdelay(5);
	g_rtc_fg_soc2=get_rtc_spare_fg_value();
	if(g_rtc_fg_soc!=g_rtc_fg_soc2){
		mdelay(3);
		printk("g_rtc_fg_soc=%d,g_rtc_fg_soc2=%d\n",g_rtc_fg_soc,g_rtc_fg_soc2);
		mdelay(2);
		g_rtc_fg_soc = get_rtc_spare_fg_value();
		}
#endif
	...
	if(plugout_status==0 && bat_is_charger_exist()==KAL_FALSE)
	{
		if(g_rtc_fg_soc==0)
		{
			if(abs(gFG_capacity_by_v_init-g_rtc_fg_soc)<=20)
					gFG_capacity_by_v=g_rtc_fg_soc;//gFG_capacity_by_v_init;//g_booting_vbat    for  0 %.
			else
...
void oam_init(void)
{
	...
	ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_ADC_V_I_SENSE, &Ibat);
	
	printk("[zoro] gFG_voltage = %d, g_booting_vbat = %d , Ibat = %d\n ",gFG_voltage,g_booting_vbat,Ibat);
	
	gFG_capacity_by_v = fgauge_read_capacity_by_v(gFG_voltage);
	vbat_capacity = fgauge_read_capacity_by_v(g_booting_vbat);
	printk("[zoro] gFG_capacity_by_v=%d, vbat_capacity=%d,\n",
	...
	printk("[zoro] gFG_capacity_by_v_init1 = %d\n",gFG_capacity_by_v_init);
	dod_init();
	printk("[zoro] gFG_capacity_by_v_init2 = %d\n",gFG_capacity_by_v_init);
...
void oam_run(void)
{
	...
	if(g_runin_battery_test_flag==0){//normal mode
		...
		oam_car_1 = (40*delta_time/3600)+ oam_car_1;
		oam_car_2 = (40*delta_time/3600)+ oam_car_2;
		}	
	}
////////////////
	if(oam_i_1>7000)
		{
		printk("oam_i_1=%d ,iam high current!\n",oam_i_1);
		gFG_BATT_CAPACITY_aging = (fgauge_get_Q_max_high_current(BMT_status.temperature)-50);
		}
...
if(g_runin_battery_test_flag==0){
///////////////////////////////
	d5_count_time = 60;
	...
	if (d5_count >= d5_count_time) {
		if (gFG_Is_Charging == KAL_FALSE) {
			...
			if (oam_d_2 > oam_d_6) {//for runin kkk2016/1/4
				oam_d_6 = oam_d_6 + 1;
			}
		} else {
			if (oam_d_5 > oam_d_3) {
			...
			if (oam_d_6 > oam_d_2) {//for runin kkk2016/1/4
				oam_d_6 = oam_d_6 - 1;
			} 
		}
		d5_count = 0;
...
/////////////////////////////////////
	...
	printk("oam_d_6=%d\n",oam_d_6);
	mdelay(2);
	bm_print(BM_LOG_CRTI, "[oam_run] %d,%d,%d,%d,%d,%d,%d,%d,%d\n",
		 d5_count, d5_count_time, oam_d_3_pre, oam_d_3, oam_d_4_pre, oam_d_4, oam_d_5,
		 charging_current,oam_d_6);
...
*/
	...
	#else
	return (100-battery_meter_trans_battery_percentage(oam_d_6));
	#endif
#endif
#endif
...
static ssize_t show_change_temp(struct device *dev,struct device_attribute *attr, char *buf)
{    
     bm_print(BM_LOG_CRTI, "[FG] jiale change temp: %d\n", change_temperature);
     return sprintf(buf, "%d\n", change_temperature);
}
static ssize_t store_change_temp(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
     char *pvalue =NULL;
     int temp = 0;
     if(buf != NULL && size != 0)
         {          
                ...
                // temp =  simple_strtoul(buf,&pvalue,10);
                temp =  simple_strtol(buf,&pvalue,10);
                 change_temperature = temp;
		if(change_temperature == 100)
			temp_control_flag = 0;
		else
			temp_control_flag = 1;
         }
...
static DEVICE_ATTR(change_temp, 0664, show_change_temp, store_change_temp);
...
static int battery_meter_probe(struct platform_device *dev)
{
	...
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_change_temp);
