...
void select_charging_curret_bcct(void)
{
	if ((BMT_status.charger_type == STANDARD_HOST) ||
		...
		g_temp_input_CC_value = CHARGE_CURRENT_1000_00_MA; //1A

		/* --------------------------------------------------- */
		/* set IOCHARGE */
		//if(g_runin_battery_test_flag==0)/////for disable the protect of tempture about charging
		//0--nomal    1--runin battery test
		//{
		if (g_bcct_value < 550)
			...
			g_temp_CC_value = CHARGE_CURRENT_975_00_MA;
