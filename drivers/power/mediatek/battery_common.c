...
static int write_rtc_count_kkk=1;
int status_kkk_charging=0;
int status_kkk_charg;
/* ///////////////////////////////////////////////////////////////////////////////////////// */
...
//@ for tempreture debug about 60'
int kkk_Test_flage;
int kkk_Test_soc_flage=10;
...
//read 
static ssize_t kkk_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
       int len = 0;
       char *p = page;
	   int volate=kkk_Test_flage;
	   int temputure=0;
    if (*ppos)  // CMD call again
    {
        return 0;
    }
	temputure=kkk_Test_flage;
     p += sprintf(p, "%d\n",temputure);//\u8bfb\u5168\u5c40\u53d8\u91cf
	*ppos += p - page;
       return (p-page);
}
...
//write
static ssize_t kkk_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
   ...
   if (copy_from_user(desc, buffer, len))
    ...
    {
        kkk_Test_flage = bat_tt_enable;  //@  
        return count;
    }
      return -EINVAL;
}

#define KKK_Test_PROC_FILE     "kkk_enable"
static struct proc_dir_entry *kkk_config_proc = NULL;
static const struct file_operations Test_proc_ops = {
    .owner = THIS_MODULE,
    .read = kkk_read_proc,
    .write = kkk_write_proc,
};
///////////////////////////////////////////////////////////////////////////////
//read 
static ssize_t kkk_read_soc_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
       int len = 0;
       char *p = page;
	   int volate=kkk_Test_soc_flage;
	   int soc=0;
    if (*ppos)  // CMD call again
    {
        return 0;
    }
	soc=kkk_Test_soc_flage;
     p += sprintf(p, "%d\n",soc);//\u8bfb\u5168\u5c40\u53d8\u91cf
	*ppos += p - page;
       return (p-page);
}
...
//write
static ssize_t kkk_write_soc_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
       int len = 0, bat_tt_enable=0;
    char desc[32];
    len = (count < (sizeof(desc) - 1)) ? count : (sizeof(desc) - 1);
   if (copy_from_user(desc, buffer, len))
    ...
    if (sscanf(desc, "%d", &bat_tt_enable) == 1)
    {
        kkk_Test_soc_flage = bat_tt_enable;  //@  
        return count;
    }
      return -EINVAL;
}

#define KKK_Test_SOC_FILE     "kkk_SOC"
static struct proc_dir_entry *kkk_SOC_config_proc = NULL;
static const struct file_operations Test_soc_ops = {
    .owner = THIS_MODULE,
    .read = kkk_read_soc_proc,
    .write = kkk_write_soc_proc,
};
...
static void mt_battery_update_EM(struct battery_data *bat_data)
{
	...
	printk("kino BMT_status.UI_SOC = %d,status_kkk_charg = %d",BMT_status.UI_SOC,status_kkk_charg);
...
static void mt_battery_Sync_UI_Percentage_to_Real(void)
{
	...
	int couneter_kkk_SOC=0;
	if(g_runin_battery_test_flag==0)
		couneter_kkk_SOC=3;//batt_cust_data.sync_to_real_tracking_time / BAT_TASK_PERIOD;
	else
		couneter_kkk_SOC=0;
	battery_log(BAT_LOG_CRTI, "[@@@@] couneter_kkk_SOC=%d\n",couneter_kkk_SOC);
	if ((BMT_status.UI_SOC > BMT_status.SOC) && ((BMT_status.UI_SOC != 1))) {
#if !defined (SYNC_UI_SOC_IMM)
		/* reduce after xxs */
		if(chr_wake_up_bat==KAL_FALSE)
		{
			if (timer_counter == couneter_kkk_SOC) 
			{
			BMT_status.UI_SOC--;
			timer_counter = 0;
			} 
...
*/
		...
		printk("kino BMT_status.UI_SOC = %d,BMT_status.SOC = %d,bat_full == %d",BMT_status.UI_SOC,BMT_status.SOC,BMT_status.bat_full);
		if(99 == BMT_status.UI_SOC && 100 == BMT_status.SOC && BMT_status.bat_full == KAL_FALSE){
			printk("kino check in BMT_status.UI_SOC = 99,BMT_status.SOC = 100,bat_full == KAL_FALSE");
		} else {
			BMT_status.UI_SOC = BMT_status.SOC;
		}
		#else
		...
		if (BMT_status.UI_SOC == -1)
			...
			if (BMT_status.UI_SOC < BMT_status.SOC && (BMT_status.SOC - BMT_status.UI_SOC > 1))
			    if(99 == BMT_status.UI_SOC && 100 == BMT_status.SOC && BMT_status.bat_full == KAL_FALSE){
					printk("kino BMT_status.SOC - BMT_status.UI_SOC > 1  BMT_status.UI_SOC = 99,BMT_status.SOC = 100,bat_full == KAL_FALSE");
					}
				else {
					BMT_status.UI_SOC++;
					}
...
static void battery_update(struct battery_data *bat_data)
{
	...
	//mdelay(3);
	//printk("batt_cust_data.sync_to_real_tracking_time=%d\n",batt_cust_data.sync_to_real_tracking_time);
	//mdelay(3);
...
void mt_battery_GetBatteryData(void)
{
	...
	status_kkk_charg=bq24296_get_chrg_stat();
	printk("kino status_kkk_charg= %d,BMT_status.UI_SOC= %d",status_kkk_charg,BMT_status.UI_SOC);
	//mdelay(5);
	//if(bat_vol>4100)
	if(status_kkk_charg==3)
	    {
	        bq24296_set_vindpm(0x0);
			//BMT_status.UI_SOC = 100%;
			//BMT_status.bat_full = KAL_TRUE;
			
	    }
	else
		bq24296_set_vindpm(0x06);
	...
	{
	//BMT_status.temperature = kkk_Test_flage;
	...
	/*--kkk-for soc test-*/
	//BMT_status.UI_SOC = kkk_Test_soc_flage;
	//BMT_status.SOC = kkk_Test_soc_flage;
	/*--kkk-for soc  test-*/
	battery_log(BAT_LOG_CRTI,
...
#endif
		battery_log(BAT_LOG_CRTI, "[BAT_thread]zoro, vibr_Enable_HW");
     ...
     else
     {
        // g_BatteryNotifyCode &= ~(0x0002);
         ...
         //if(temperature <= MIN_CHARGE_TEMPERATURE) 
                 if(temperature < MIN_CHARGE_TEMPERATURE)   // liguangxian 20141016
     {
...
void BAT_thread(void)
{
	...
	printk("kino run into BAT_thread");
...
int bat_thread_kthread(void *x)
{
	...
	printk("run into bat_thread_kthread");
...
#endif
		kkk_config_proc = proc_create(KKK_Test_PROC_FILE, 0666, NULL, &Test_proc_ops);
/////////////////////SOC fang_zhen////////////////
		kkk_SOC_config_proc = proc_create(KKK_Test_SOC_FILE, 0666, NULL, &Test_soc_ops);
  	if (kkk_config_proc == NULL)
     	printk("create kkk_proc_entry %s failed\n", KKK_Test_PROC_FILE);
  	else
     	printk("create kkk_proc entry %s success", KKK_Test_PROC_FILE);
	}
