...
//static volatile int gsl_power_flag = 0;
...
unsigned int gsl_model_extern[] ={
	...
	0x10,'E',
	0xa3916a7c,0xc6b7475a,0xc8cf162e,0x9bb40007,0x6a830300,0x3c52150b,0x17273523,0x050c654c,
	0x0101967d,0x0e06c6af,0x2f1cecdc,0x5f46fcf7,0x9077fbff,0xbda7e6f2,0xe1d0c3d6,0xffef97ae,
	0x10, 'E',
	0x3c24aebd,0x6b5490a0,0x97816d80,0xb3a63b56,0xa5b30620,0x6f8a0c00,0x43592e1c,0x20305b43,
	0x07128e73,0x0002c5a9,0x1703f4df,0x4e32fdff,0x8269e8f6,0xaf98c7d9,0xd7c39fb4,0xffe9758b,
	0x10,'E',
	0x462c7f84,0x79606d79,0xa38e4d5f,0xbeb41e38,0x9bb60007,0x67810702,0x364d1d10,0x0d1f422e,
	0x0205765c,0x0100ab91,0x1609ddc5,0x4229fdf1,0x775cf7fe,0xa68eddec,0xd3bdbecf,0xffe89ead,
	0x10,0x1,
	...
	0x10,0x1,
	0xe6d6e2f0,0xf9f1bcd0,0xfffe93a7,0xf7fc6a7e,0xe4ee4657,0xcbd82434,0xa9bb0c17,0x81950004,
	0x586c0300,0x3545180c,0x18263727,0x040c5c49,0x00018571,0x0300ae9a,0x0e08d7c3,0x2318ffeb,
	0x10,' ',
	0x20320b00,0x06102e1a,0x00025943,0x0401836e,0x1109ad98,0x281cd2c0,0x4837eee1,0x715cfcf7,
	0x9b86ffff,0xc4b1f3fb,0xe7d7dae8,0xf9f1b2c7,0xfffd889d,0xfcff5d73,0xf3f93348,0xdc080c20,
	0x10,0x1,
	0x6d6d1e00,0x6d6d5b3d,0x6e6d987a,0x7e73d4b7,0xab8ffef0,0xe4cae9fa,0xfcf3afcd,0xfdff7290,
	0xe2f03a55,0xb5ce0f21,0x79970104,0x3d5a0900,0x1324361c,0x02077153,0x0200ae90,0x1c0be9cc,
	0x10,0x1,
	0x9999d9f8,0x97989ab9,0x90965b7a,0x7f8a1c3b,0x48680006,0x0d291704,0x00045535,0x07019375,
	0x2112ceb2,0x5638f5e6,0x9575fffc,0xd1b4f0fc,0xf5e7bbd9,0xfffc7c9b,0xf3fc3e5c,0xd4e7021f,
...
static void gsl_report_point(struct gsl_touch_info *ti)
{
	...
	else 
	{
		...
		for (tmp = 0; ti->finger_num > tmp; tmp++) 
		{
			...
			{
			input_report_abs(tpd->dev, ABS_MT_POSITION_X,(((ti->x[tmp])*605+300)/600));
			}
...
//	}
	...
	{
	    printk("kino: gsl_report_work: tpd_proximity_flag == 1");
	
		gsl_read_interface(ddata->client,0xac,buf,4);
		printk("kino: tpd_proximity_detect buf[0]%d,buf[1]%d,buf[2]%d,buf[3]%d",buf[0],buf[1],buf[2],buf[3]);
                printk("[tp-gsl] proximity_state  ac read_buf = %02x%02x%02x%02x\n", buf[3], buf[2], buf[1], buf[0]);
		if (buf[0] == 1 && buf[1] == 0 && buf[2] == 0 && buf[3] == 0)
		{
			tpd_proximity_detect = 0;
			printk("kino: gsl_report_work: tpd_proximity_detect = 0;");
			//sensor_data.values[0] = 0;
		}
		else
		{
			tpd_proximity_detect = 1;
			printk("kino: gsl_report_work: tpd_proximity_detect = 1;");
		...
		{
			printk("kino: gsl_report_work: hwmsen_get_interrupt_data OK;");
		}
	}
	/*end of added*/
#endif
	...
	printk("tp-gsl  idqian x = %d y = %d finger_num = %d\n",cinfo.x[0],cinfo.y[0],cinfo.finger_num);
        gsl_alg_id_main(&cinfo);
	printk("tp-gsl idhou  x = %d y = %d finger_num = %d \n",cinfo.x[0],cinfo.y[0],cinfo.finger_num);
	...
	if(GE_ENABLE == gsl_gesture_status && gsl_gesture_flag == 1){
		...
		if(key_data != 0){
			...
			msleep(2);
			do_gettimeofday(&time_last);
			printk("time_last_sec = %ld     time_last_usec = %ld\n",time_last.tv_sec,time_last.tv_usec);
...
static void gsl_hw_init(void)
{
	...
	//hwPowerOn(MT6325_POWER_LDO_VGP2, VOL_2800, "TP");
	pmic_set_register_value(PMIC_RG_VGP2_VOSEL,5); //2.8V
...
//gesture up
		gt91xxgup_config_proc = proc_create(GT91XXGUP_CONFIG_PROC_FILE, 0666, NULL, &configgup_proc_ops);
		   if (gt91xxgup_config_proc == NULL)
			 print_info("create_proc_entry %s failed\n", GT91XXGUP_CONFIG_PROC_FILE);
		  else
			 print_info("create proc entry %s success", GT91XXGUP_CONFIG_PROC_FILE);
//gesture down
		gt91xxgdo_config_proc = proc_create(GT91XXGDO_CONFIG_PROC_FILE, 0666, NULL, &configgdo_proc_ops);
			 if (gt91xxgdo_config_proc == NULL)
			   print_info("create_proc_entry %s failed\n", GT91XXGDO_CONFIG_PROC_FILE);
			else
			   print_info("create proc entry %s success", GT91XXGDO_CONFIG_PROC_FILE);
//gesture right
	gt91xxgri_config_proc = proc_create(GT91XXGRI_CONFIG_PROC_FILE, 0666, NULL, &configgri_proc_ops);
	   if (gt91xxgri_config_proc == NULL)
		 print_info("create_proc_entry %s failed\n", GT91XXGRI_CONFIG_PROC_FILE);
	  else
		 print_info("create proc entry %s success", GT91XXGRI_CONFIG_PROC_FILE);
//gesture left
	gt91xxgle_config_proc = proc_create(GT91XXGLE_CONFIG_PROC_FILE, 0666, NULL, &configgle_proc_ops);
		 if (gt91xxgle_config_proc == NULL)
		   print_info("create_proc_entry %s failed\n", GT91XXGLE_CONFIG_PROC_FILE);
		else
		   print_info("create proc entry %s success", GT91XXGLE_CONFIG_PROC_FILE);
//gesture double
			gt91xxgdoub_config_proc = proc_create(GT91XXGDOUB_CONFIG_PROC_FILE, 0666, NULL, &configgdoub_proc_ops);
				 if (gt91xxgdoub_config_proc == NULL)
	...
	ID_tmp[0]=0x3;ID_tmp[1]=0;ID_tmp[2]=0;ID_tmp[3]=0;
	gsl_write_interface(ddata->client,0xf0,ID_tmp,4);
	gsl_read_interface(ddata->client,4,ID_tmp,4);
...
static void gsl_suspend(struct i2c_client *client)
{
	...
	{
	gsl_gesture_status = GE_ENABLE;
	printk("kino: gsl_suspend: gsl_gesture_status = GE_ENABLE");
	return 0;
	}
#endif
	gsl_halt_flag = 1;
	...
	if(gsl_gesture_flag == 1){
		...
		printk("kino: gsl_resume: gsl_enter_doze");
		return;
	}
#endif
	printk("[zoro] not in gsl_enter_doze!\n");
	mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
//	gsl_reset_core(ddata->client);
//	msleep(20);
	mt_set_gpio_out(GPIO_CTP_RST_PIN, GPIO_OUT_ZERO);
...
static void gsl_resume(struct i2c_client *client)
{
	...
	if (tpd_proximity_flag == 1&&gsl_halt_flag == 0)
	{
		gsl_gesture_status = GE_DISABLE;
		tpd_enable_ps(1);
		printk("kino: gsl_resume: tpd_enable_ps(1) and gsl_gesture_status = GE_DISABLE;");
		return;
	}
#endif   
	...
	if(gsl_gesture_flag == 1){
		gsl_quit_doze(ddata);
		printk("kino: gsl_resume: gsl_quit_doze");
	}
#endif
	...
	{
		tpd_enable_ps(1);
		printk("kino: gsl_resume: tpd_enable_ps(1)");
		return;
	}
#endif
